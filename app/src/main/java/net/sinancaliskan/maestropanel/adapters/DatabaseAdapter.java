...
import net.sinancaliskan.maestropanel.ui.DatabaseUserManagement;
...
public class DatabaseAdapter extends RecyclerView.Adapter<DatabaseAdapter.DatabaseHolder> implements SwipeableItemAdapter<DatabaseAdapter.DatabaseHolder> {
    ...
    //region viewHolder
    public static class DatabaseHolder extends AbstractSwipeableItemViewHolder {
        View swipeable_container,editUserManagement,editQuota,delete;
        Database model;
        ImageView status;
        TextView database,limit,size;
        float lastSwipeAmount;
        public DatabaseHolder(final View itemView)
        {
            super(itemView);

            swipeable_container=itemView.findViewById(R.id.swipeable_container);
            editUserManagement=itemView.findViewById(R.id.editUserManagement);
            editQuota=itemView.findViewById(R.id.editQuota);
            delete=itemView.findViewById(R.id.delete);

            status= (ImageView) itemView.findViewById(R.id.status);
            database=(TextView) itemView.findViewById(R.id.txtDatabase);
            limit=(TextView) itemView.findViewById(R.id.txtLimit);
            size=(TextView) itemView.findViewById(R.id.txtSize);

            swipeable_container.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent=new Intent(context, DatabaseUserManagement.class);
                    intent.putExtra("model", domain);
                    intent.putExtra("database", model);
                    context.startActivity(intent);
                }
            });
            editUserManagement.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent=new Intent(context, DatabaseUserManagement.class);
                    intent.putExtra("model", domain);
            ...
            editQuota.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    new DatabaseChangeQuota(context,adapter,model);
                }
            });
            ...
            delete.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    new DatabaseDelete(context,adapter,model.dbType,model.name);
                }
            });
        }
        @Override
        ...
        public void onSlideAmountUpdated(float horizontalAmount, float verticalAmount, boolean isSwiping) {
            ...
            if (editUserManagement.getWidth() == 0) {
                setLayoutWidth(editUserManagement, (int) (optionItemWidth + 0.5f));
                setLayoutWidth(editQuota, (int) (optionItemWidth + 0.5f));
                setLayoutWidth(delete, (int) (optionItemWidth + 0.5f));
            }
            editUserManagement.setTranslationX(-(int) (p * optionItemWidth * 3 + 0.5f) + offset);
            ...
            if (horizontalAmount < (-REMOVE_ITEM_THRESHOLD)) {
                ...
                editUserManagement.setVisibility(View.INVISIBLE);
                editQuota.setVisibility(View.INVISIBLE);
                delete.setVisibility(View.INVISIBLE);
            } else {
                swipeable_container.setVisibility(View.VISIBLE);
                editUserManagement.setVisibility(View.VISIBLE);
    ...
    //region adapter
    static String TAG=DatabaseAdapter.class.getSimpleName();
    private static ArrayList<Database> models=new ArrayList<>();
    ...
    public DatabaseAdapter(Context context,SwipeRefreshLayout swipeRefreshLayout, RecyclerView recyclerView, Domain model){
        ...
        progressDialog.setMessage(context.getString(R.string.database)+ context.getString(R.string.isLoading));
    ...
    public DatabaseHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View row= LayoutInflater.from(parent.getContext()).inflate(R.layout.row_database,parent,false);
        return new DatabaseHolder(row);
    }

    @Override
    public void onBindViewHolder(DatabaseHolder holder, int position) {
        Database model=models.get(position);
        holder.model=model;
        Drawable image=null;
        if (model.dbType.equals("mssql"))
            image= ContextCompat.getDrawable(context, R.drawable.mssql);
        else if (model.dbType.equals("mysql"))
            image= ContextCompat.getDrawable(context, R.drawable.mysql);
        holder.status.setImageDrawable(image);
        holder.database.setText(model.name.toString());
        holder.limit.setText(model.diskQuota.toString());
        holder.size.setText(model.diskUsage.toString());
    ...
    public int getItemPosition(String dbType,String model){
        int position=-1;
        for (int i = 0; i < models.size(); i++) {
            if (models.get(i).name==model && models.get(i).dbType==dbType)
                return i;
        }
        return position;
    }
    ...
    public void removeItem(String dbType,String model){
        int position=getItemPosition(dbType,model);
        if (position>-1)
            removeItem(position);
    }

    public void updateItem(Database model,int quota){
        model.diskQuota=quota;
        adapter.notifyItemChanged(getItemPosition(model.dbType,model.name));
    }

    @Override
    public SwipeResultAction onSwipeItem(DatabaseHolder holder, int position, int result) {
    ...
    @Override
    public int onGetSwipeReactionType(DatabaseHolder holder, int position, int x, int y) {
        return Swipeable.REACTION_CAN_SWIPE_LEFT;
    }

    @Override
    public void onSetSwipeBackground(DatabaseHolder holder, int position, int type) {
    ...
    static class SwipeLeftPinningAction extends SwipeResultActionMoveToSwipedDirection {
        ...
        public SwipeLeftPinningAction(DatabaseAdapter adapter, int position) {
    ...
    static class SwipeCancelAction extends SwipeResultActionDefault {
        ...
        public SwipeCancelAction(DatabaseAdapter adapter, int position) {
            this.adapter = adapter;
            this.position = position;
        }
...
//                        adapter.notifyItemInserted(0);
    ...
    boolean checkItemExist(Database model){
        boolean exist=false;
        for (int i=0;i<models.size();i++){
            if (models.get(i).name.equals(model.name) && models.get(i).dbType.equals(model.dbType) ){exist=true;break;}
        }
        return exist;
    }
    //endregion

...
import net.sinancaliskan.maestropanel.models.pojo.ServerObjects.IPAddress;
...
public class ServerIPAdapter extends RecyclerView.Adapter<ServerIPAdapter.IPAddressHolder> implements SwipeableItemAdapter<ServerIPAdapter.IPAddressHolder> {
    ...
    public static class IPAddressHolder extends AbstractSwipeableItemViewHolder {
        ...
        IPAddress model;
        TextView nic,ipAddress,subnet,domainCount;
        ...
        public IPAddressHolder(View itemView)
        {
            ...
            subnet=(TextView) itemView.findViewById(R.id.txtSubnet);
            domainCount=(TextView) itemView.findViewById(R.id.txtDomainCount);
    ...
    private static ServerIPAdapter adapter;
    ...
    public ServerIPAdapter(Context context, SwipeRefreshLayout swipeRefreshLayout, RecyclerView recyclerView, Server model){
    ...
    public IPAddressHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View row= LayoutInflater.from(parent.getContext()).inflate(R.layout.row_server_ip_address,parent,false);
    ...
    public void onBindViewHolder(IPAddressHolder holder, int position) {
        ...
        holder.nic.setText(model.nicName);
        holder.ipAddress.setText(model.ipAddr);
        holder.subnet.setText(model.subnet);
        holder.domainCount.setText(model.domainCount.toString());
    ...
    static class SwipeLeftPinningAction extends SwipeResultActionMoveToSwipedDirection {
        ...
        public SwipeLeftPinningAction(ServerIPAdapter adapter, int position) {
    ...
    static class SwipeCancelAction extends SwipeResultActionDefault {
        ...
        public SwipeCancelAction(ServerIPAdapter adapter, int position) {
            this.adapter = adapter;
            this.position = position;
        }
    ...
    public void refresh() {
        ...
        Call<Reply> call=service.GetIpAddrList(key);
    ...
    boolean checkItemExist(IPAddress model){
        boolean exist=false;
        for (int i=0;i<models.size();i++){
            if (models.get(i).id.equals(model.id)){exist=true;break;}
        }
        return exist;
    }
    //endregion

...
public class ResellerPasswordChange {
    ...
    public ResellerPasswordChange(final Context context, final Reseller model) {
        ...
        final View view=layoutInflater.inflate(R.layout.dialog_reseller_change_password,null);        //create view with dialog_domain_change_password layout
        ...
        builder.setPositiveButton(R.string.change_password, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {                            //set dialog positivebutton with name and click method
                ...
                String username=model.username,newPassword=txtNewPassword.getText().toString();       //set string for domain change password
                service = API.getClient().create(IReseller.class);                                    //crete service for domain
                call=service.ChangePassword(key,username,newPassword);                            //create service call method with parameters
                ...
                call.enqueue(new Callback<Reply>() {                                                //adding queue to callback
                    ...
                    public void onResponse(Call<Reply> call, Response<Reply> response) {            //when callback return response
                        try {                                                                       //show notification action is completed
                            Notification.Show(context,R.drawable.ic_supervisor_account_white_24dp,R.string.change_complated,response.body().message);
                        }
                        catch (Exception e){                                                        //show notification action isn't completed
                            Notification.Show(context,R.drawable.ic_supervisor_account_white_24dp,R.string.change_operation,e.getMessage());
                        }
                    }
                    @Override
